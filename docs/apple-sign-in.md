...
生产已配置 Key ID `9Q234VQTL6`，`auth/capabilities` 返回 `appleSignInEnabled=true`，内网继续只提供测试账号。已校验私钥格式、服务端加载和接口就绪；真实 Apple 授权、同步和删除流程仍待真机验收，不能以配置就绪代替登录成功。
...
`cmd/configure-apple` 从标准输入读取 `.p8`，先校验 P-256/标识，再保留已有加密密钥及其他环境设置，创建 600 权限的变更前备份，最后原子替换环境文件。密钥不出现在命令参数或输出中。Git、Docker 构建与部署同步均排除私钥文件；新增凭据加密密钥的异机加密备份仍属于上线运维门禁。
