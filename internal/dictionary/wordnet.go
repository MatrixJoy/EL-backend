...
func ImportWordNet31(ctx context.Context, pool *pgxpool.Pool, archive io.Reader) (ImportResult, error) {
	...
	for _, required := range []string{"data.noun", "data.verb", "data.adj", "data.adv", "noun.exc", "verb.exc", "adj.exc", "adv.exc"} {
		if !found[required] {
			return ImportResult{}, fmt.Errorf("WordNet archive is missing %s", required)
		}
	}
	if _, err = tx.Exec(ctx, `
		DELETE FROM dictionary_entries WHERE source = $1;
		...
		SELECT normalized_word, word, part_of_speech, definition, examples,
			$1, source_reference, 100
		FROM (
			SELECT DISTINCT ON (normalized_word, part_of_speech, definition)
				* FROM dictionary_import_entries
			ORDER BY normalized_word, part_of_speech, definition, source_reference
		) entries
		...
		INSERT INTO dictionary_forms (form, lemma, part_of_speech)
		SELECT DISTINCT form, lemma, part_of_speech FROM dictionary_import_forms
		ON CONFLICT DO NOTHING`, SourceWordNet31); err != nil {
		return ImportResult{}, err
	}
...
func parseDataLine(line, partOfSpeech string) ([]wordNetEntry, bool, error) {
	...
	for index := 0; index < int(wordCount); index++ {
		...
		entries = append(entries, wordNetEntry{
			...
			Examples:       examples,
			SourceRef:      partOfSpeech[:1] + ":" + fields[0],
		})
	}
